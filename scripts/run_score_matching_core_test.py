"""Run Gaussian score-matching CORe MIQP on a saved dataset."""
...
def parse_args() -> argparse.Namespace:
    ...
    add_dataset_runner_arguments(
        parser,
        Path(os.path.join(PROJECT_DIR, "results", "score_matching_core", "pycharm_default.csv")),
    )
    parser.set_defaults(data_source="exact", m=20, n=500, target_edges=30, seed=0)
    ...
    try:
        ...
        return result_row(
            ...
            **metrics,
            ...
            kappa_min=float(solution.thresholds.kappa.min()),
            kappa_max=float(solution.thresholds.kappa.max()),
            tau_min=float(solution.thresholds.tau.min()),
            tau_max=float(solution.thresholds.tau.max()),
