import importlib.util
import os
import sys
import unittest

import numpy as np


current_dir = os.path.dirname(os.path.abspath(__file__))
project_dir = os.path.dirname(current_dir)


def load_src_module(module_name: str):
    module_path = os.path.join(project_dir, "src", f"{module_name}.py")
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
    return module


score_matching_miqp = load_src_module("score_matching_miqp")
score_matching_core_miqp = load_src_module("score_matching_core_miqp")
utils = load_src_module("utils")

build_gaussian_score_matching_formulation = (
    score_matching_miqp.build_gaussian_score_matching_formulation
)
reconstruct_precision = score_matching_miqp.reconstruct_precision
core_thresholds = score_matching_core_miqp.core_thresholds
solve_score_matching_core_miqp = score_matching_core_miqp.solve_score_matching_core_miqp
generate_exact_m_gaussian_data = utils.generate_exact_m_gaussian_data


class ScoreMatchingCOReMIQPTest(unittest.TestCase):
    def small_data(self):
        x, _, _, _ = generate_exact_m_gaussian_data(
            n=40,
            m=5,
            target_edges=5,
            seed=3,
        )
        return x

    def test_core_thresholds_are_positive_and_finite(self):
        formulation = build_gaussian_score_matching_formulation(self.small_data())
        thresholds = core_thresholds(formulation.Q_prof, lambda_value=0.01)

        n_edges = 5 * 4 // 2
        self.assertEqual(len(formulation.edge_list), n_edges)
        self.assertEqual(thresholds.q_diag.shape, (n_edges,))
        self.assertEqual(thresholds.kappa.shape, (n_edges,))
        self.assertEqual(thresholds.tau.shape, (n_edges,))
        self.assertTrue(np.all(thresholds.q_diag > 0))
        self.assertTrue(np.all(thresholds.kappa > 0))
        self.assertTrue(np.all(thresholds.tau > 0))
        self.assertTrue(np.all(np.isfinite(thresholds.kappa)))
        self.assertTrue(np.all(np.isfinite(thresholds.tau)))

    def test_reconstructed_precision_is_symmetric(self):
        formulation = build_gaussian_score_matching_formulation(self.small_data())
        beta = np.linspace(-0.2, 0.2, len(formulation.edge_list))

        precision = reconstruct_precision(beta, formulation)

        self.assertEqual(precision.shape, (5, 5))
        self.assertTrue(np.allclose(precision, precision.T))

    def test_solver_smoke_if_gurobi_is_available(self):
        try:
            import gurobipy  # noqa: F401
        except ImportError as exc:
            self.skipTest(f"gurobipy is not installed: {exc}")

        try:
            solution = solve_score_matching_core_miqp(
                self.small_data(),
                lambda_value=0.01,
                time_limit=5,
                mip_gap=0.1,
            )
        except Exception as exc:
            message = str(exc).lower()
            if "license" in message or "gurobi" in message:
                self.skipTest(f"Gurobi smoke test skipped: {exc}")
            raise

        self.assertTrue(solution.status)
        self.assertEqual(solution.precision.shape, (5, 5))
        self.assertTrue(np.allclose(solution.precision, solution.precision.T))
        self.assertTrue(np.allclose(solution.adjacency, solution.adjacency.T))


if __name__ == "__main__":
    unittest.main()
