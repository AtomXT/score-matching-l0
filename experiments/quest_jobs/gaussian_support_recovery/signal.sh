#!/bin/bash
# Vary the minimum partial-correlation target and record the achieved value.
...
#SBATCH --mem=12G
#SBATCH --array=0-99
#SBATCH --job-name=sm_signal
#SBATCH --output=experiments/quest_jobs/outlog/sm_signal_%A_%a.log
...
python3 -m experiments.Run_gaussian_experiments \
  ...
  --mip-gap "0.01" \
  --threads "8" \
  --job-name "signal_rep${SLURM_ARRAY_TASK_ID}" \
  --overwrite-results
